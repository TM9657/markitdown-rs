...
use crate::model::{ContentBlock, ConversionOptions, Document, DocumentConverter, ExtractedImage, Page};
...
impl DocConverter {
    fn convert_doc(bytes: &[u8], options: Option<&ConversionOptions>) -> Result<Document, MarkitdownError> {
        ...
        if extract_images {
            ...
            if let Ok(mut cfb) = CompoundFile::open(cursor) {
                ...
                for image in images {
                    ...
                    extracted.alt_text = Some(format!("image{}.{}", image.index, image.format.extension()));
    ...
    fn extract_images_from_doc<R: Read + std::io::Seek>(cfb: &mut CompoundFile<R>) -> Vec<LegacyImage> {
    ...
    fn try_extract_image_at(data: &[u8], pos: usize) -> Option<(Vec<u8>, ImageFormat)> {
        ...
        // EMF
        if slice.len() >= 44 && slice[0..4] == [0x01, 0x00, 0x00, 0x00] && slice[40..44] == [0x20, 0x45, 0x4D, 0x46] {
    ...
    fn extract_text_from_word_document(data: &[u8]) -> Option<String> {
        ...
        if data.len() > 0x50 {
            let ccp_text = u32::from_le_bytes([data[0x4C], data[0x4D], data[0x4E], data[0x4F]]) as usize;
            ...
            if data.len() > 0x1C {
                let fc_min = u32::from_le_bytes([data[0x18], data[0x19], data[0x1A], data[0x1B]]) as usize;
    ...
    fn find_ansi_text_runs(data: &[u8]) -> Option<String> {
        ...
        for &byte in data {
            ...
            if c.is_alphanumeric() || c.is_whitespace() || ".,;:!?()-\"'äöüÄÖÜßéèêëàâáíìîïóòôõúùûñçÑ".contains(c) {
                run.push(c);
...
impl PptConverter {
    fn convert_ppt(bytes: &[u8], options: Option<&ConversionOptions>) -> Result<Document, MarkitdownError> {
        let cursor = Cursor::new(bytes);
        let mut cfb = CompoundFile::open(cursor)
        ...
        if extract_images {
            ...
            if let Ok(mut cfb) = CompoundFile::open(cursor) {
                ...
                for image in images {
                    ...
                    extracted.alt_text = Some(format!("slide_image{}.{}", image.index, image.format.extension()));
                    extracted.page_number = Some(1);
                    page.add_content(ContentBlock::Image(extracted));
                }
            }
    ...
    fn extract_images_from_ppt<R: Read + std::io::Seek>(cfb: &mut CompoundFile<R>) -> Vec<LegacyImage> {
        let mut images = Vec::new();
    ...
    fn extract_text_from_ppt_stream(data: &[u8]) -> Option<String> {
        ...
        if !texts.is_empty() {
            ...
            let unique: Vec<_> = texts.into_iter()
    ...
    fn is_meaningful_text(text: &str) -> bool {
        ...
        let has_garbage = text.contains("\x00") || 
                         ...
                         text.chars().filter(|c| !c.is_ascii_graphic() && !c.is_whitespace()).count() > total_count / 4;
